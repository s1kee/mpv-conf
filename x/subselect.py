import sys, os
sys.path.append('C:\\Users\\Therese\\AppData\\Local\\Packages\\PythonSoftwareFoundation.Python.3.7_qbz5n2kfra8p0\\LocalCache\\local-packages\\Python37\\site-packages')
from tkinter import *
...
class subselect :
    ...
    def search(self) :
        try :
            ...
            subtitles = list_subtitles([self.video], {Language(self.language)}, providers=providers_list, provider_configs=providers_auth)
    ...
    def download_best_subtitle(self) :
        try :
            self.video = self.get_video_from_title()
            best_subtitles = download_best_subtitles([self.video], {Language(self.language)}, providers=providers_list, provider_configs=providers_auth)
        except ValueError as exc :
            self.show_message("Error", str(exc))
        else :
...
videotitle = save_dir = ""
sub_language = "eng"
providers_list = ['addic7ed', 'opensubtitles', 'podnapisi']
providers_auth = {'addic7ed': {'username': 'bingewatcher', 'password': 'oddsbodikins'},'opensubtitles': {'username': 'jelaixx', 'password': 'yK#xNXPHjzVe4w'}}
