...
class subselect :
    ...
    def download_best_subtitle(self) :
        try :
            self.video = self.get_video_from_title()
            best_subtitles = download_best_subtitles([self.video], {Language(self.language)}, providers=None, provider_configs=providers_auth)
        except ValueError as exc :
            self.show_message("Error", str(exc))
        else :
...
videotitle = save_dir = ""
sub_language = "eng"
providers_auth = {'addic7ed': {'username': 'bingewatcher', 'password': 'oddsbodikins'},'opensubtitles': {'username': 'bingewatcher7', 'password': 'bDw@f68y'}}
