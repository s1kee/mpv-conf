...
function activate_script()
    ...
    local python_cmd = string.format('start /MIN "" cmd /C "python "%s/rate-watching.py" --activate"', script_dir)
...
mp.add_key_binding('j', 'trakt-rating-watching', function()
    mp.add_timeout(1, activate_script)
end)